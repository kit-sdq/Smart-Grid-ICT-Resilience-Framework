...
public class LocalHacker implements IAttackerSimulation {
	...
	public ErrorCodeEnum init(ILaunchConfiguration config) throws CoreException {
		...
		HackingStyle desiredHackingStyle;
		int rootNodeID;
		int hackingSpeed;
		...
		//Extracting Parameters from Config
			...
			desiredHackingStyleString = config.getAttribute(
					Constants.HACKING_STYLE_KEY, Constants.FAIL);

			rootNodeIDString = config.getAttribute(Constants.ROOT_NODE_ID_KEY,
					Constants.FAIL);

			hackingSpeedString = config.getAttribute(Constants.HACKING_SPEED_KEY,
					Constants.FAIL);
		...
		if (desiredHackingStyleString.equals(Constants.FAIL)
				|| rootNodeIDString.equals(Constants.FAIL)
				|| hackingSpeedString.equals(Constants.FAIL)) {

			errorCode = ErrorCodeEnum.DEFAULT_VALUES_USED;

			desiredHackingStyle = HackingStyle
					.valueOf(Constants.DEFAULT_HACKING_STYLE);

			hackingSpeed = Integer.parseUnsignedInt(Constants.DEFAULT_HACKING_SPEED);
		...
		{
			desiredHackingStyle = HackingStyle
					.valueOf(desiredHackingStyleString);

			hackingSpeed = Integer.parseUnsignedInt(hackingSpeedString);
	 ...
	 *            Value is 1.
	 */
	public LocalHacker(HackingStyle desiredHackingStyle, int rootNodeID,
			int hackingSpeed) {
	...
	public ScenarioResult run(Scenario smartGridTopo,
		...
		// Find Root On Entity using ID I already know
			...
			this.rootNode = ScenarioHelper.findEntityOnStateFromID(rootNodeID,
					myScenarioResult);
		}
		...
		assert (this.rootNode != null && this.rootNode.getOwner().getId() == this.rootNodeID) : "Root Node Not Valid !";
...
//		usedHackingStyle = HackingStyle.FULLY_MESHED_HACKING;
	...
	private void bfsHacking(Cluster clusterToHack) {
		...
		hackingDone: for (layerCount = 0; !currentLayer.isEmpty(); layerCount++) { // Change
			...
			for (On Node : currentLayer) {
				...
				LinkedList<Integer> neighborIDList = IDtoHisNeighborLinks
						.get(nodeID);

				/*
				 * Here it Filters the hardwired Logical Connections of the
				 * Neighbors out that because of their State (e.g. destroyed)
				 * don't functions
				 */
				LinkedList<On> neighborOnList = ScenarioHelper
						.getNeighborsFromCluster(clusterToHack, neighborIDList);
	...
	// getNeighborsFromCluster() outsourced into Helper Class --> {link
	// smartgrid.helper.ScenarioHelper}
	...
	// * @param neighborOnList
	// */
	// private LinkedList<On> getNeighborsFromCluster(Cluster clusterToHack,
	...
	// if (neighborIDList
	// .contains(ScenarioHelper.getIDfromEntityOnState(clusterNode))) {
	//
	...
	// genNeighborMapbyID() Outsourced into Helper Class --> {link
	// smartgrid.helper.ScenarioHelper}

	/*
	 * Hacks like a Deep First Search
	 * 
	 * DFS Algorithm
	 * 
	 * S := RootNode; int hackCount = 0;
	 * 
	 * hack Root
	 * 
	 * DFS (S);
	 * 
	 * 
	 * Procedure DFS(v : Node) foreach Neighbor of v {
	 * 
	 * if(hackCount > this.hackingSpeed ) --> Break;
	 * 
	 * 
	 * if(Neighbor.isHacked) { DFS(Neighbor);
	 * 
	 * } else { Hack Neighbor; DFS(Neighbor);
	 * 
	 * }
	 * 
	 * }
	 ...
	 */
	private void dfs(Cluster clusterToHack, On Node,
			Map<Integer, LinkedList<Integer>> IDtoHisNeighborLinks,
			int hackCount) {
		// getting Neighbors
		int nodeID = ScenarioHelper.getIDfromEntityOnState(Node);
		 ...
		 */
		LinkedList<On> neighborOnList = ScenarioHelper.getNeighborsFromCluster(
				clusterToHack, neighborIDList);
		...
		case -1:
			...
			do {
				int myClusterNumber = myRandom.nextInt(clusterCount); // [0 -
																		...
																		// upper
																		// bound
				myCluster = this.myScenarioResult.getClusters().get(
						myClusterNumber);
	...
	/*-->Outsourced {@link ScenarioHelper}
	...
	// NetworkEntity ID
	// */
	// private On findEntityOnStateFromID(int entityID){
	//
	//
	...
	//
	//
	// for(smartgridoutput.EntityState currentNode :
	// myScenario.getEntityStates()){ //Alternative with foreach Cluster foreach
	// On Entities ?? (Double foreach Loop! )
	//
	// if(currentNode.getOwner().getId() == entityID && currentNode instanceof
	// On) //Using only Entities that is On
	//
	// {
	...
	// private int getIDfromEntityOnState(On eintityOnState){
	//
