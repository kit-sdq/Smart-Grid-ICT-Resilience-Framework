...
public class ViralHacker implements IAttackerSimulation {
	 ...
	 * <p>
	 * 
	 * Remark Root NodeIDs {@link smartgrid.simcontrol.baselib.launchconfig.Constants} have to be
	 * List of String !
	 * 
	 * 
	 * 
	...
	public ErrorCodeEnum init(ILaunchConfiguration config) throws CoreException {
		...
		myModeString = config
				.getAttribute(Constants.NODE_MODE, Constants.FAIL);
		...
		// Getting Hacking Speed
		String hackingSpeedString = config.getAttribute(
				Constants.HACKING_SPEED_KEY, Constants.FAIL);
		if (hackingSpeedString.equals(Constants.FAIL)) {
			defaultUsed = true;
			hackingSpeedString = Constants.DEFAULT_HACKING_SPEED;
		}
		...
		this.usedHackingStyle = HackingStyle
				.valueOf(hackingStyleString);
		...
		case NodeIDs:
			...
			//Getting NodeID 
			List<String> nodeIDStrings = config.getAttribute(
					Constants.ROOT_NODE_ID_KEY, fail);
		...
		case Nodes:
			...
			String nodePath = config.getAttribute(Constants.NODE_PATH,
					Constants.FAIL);
			...
			hackingDone: for (layerCount = 0; !currentLayer.isEmpty(); layerCount++) { // Change
				...
				for (On Node : currentLayer) {
					...
					LinkedList<On> neighborOnList = ScenarioHelper
							.getNeighborsFromCluster(clusterToHack,
									neighborIDList);
