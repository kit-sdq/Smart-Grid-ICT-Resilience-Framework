...
public class GraphAnalyzer implements IImpactAnalysis {
	...
	public ErrorCodeEnum init(ILaunchConfiguration config) throws CoreException {
		...
		String ignoreLogicalConnectionsString = config.getAttribute(
				Constants.IGNORE_LOC_CON_KEY, Constants.FAIL);
		...
		if (ignoreLogicalConnectionsString.equals(Constants.FAIL)) {
			...
			this.ignoreLogicalConnections = (Constants.TRUE)
					.equals(Constants.DEFAULT_IGNORE_LOC_CON);
		} else {
			//checks whether ignoreLogicalConnectionsString is true and assigns it
			this.ignoreLogicalConnections = (Constants.TRUE)
					.equals(ignoreLogicalConnectionsString);
		}
...
//		}//TODO Remove if for rollback
	...
	private void readPhysicalConnections(Scenario scenario, ScenarioState state) {
		...
		for (PhysicalConnection p : pConns) {
			...
			for (NetworkEntity e : p.getLinks()) {
				System.out.println(e.getClass() + " " + e.getName() + " ID "
						+ e.getId());
			}
			NetworkEntity e1 = p.getLinks().get(0);
			NetworkEntity e2 = p.getLinks().get(1);
			if (externalNodeIsWorking(e1.getId())
					&& externalNodeIsWorking(e2.getId())) {
		...
		physicalClusters = Tarjan.getClusters(adjacentMatrix,
	...
	private void readLogicalConnections(Scenario scenario, ScenarioState state) {
		...
		if(!ignoreLogicalConnections){
			...
			logicalClusters = Tarjan.getClusters(logicalAdjacentMatrix,
				internalToExternalID);
		}else{
		...
		for (List<Integer> cluster : logicalClusters) { //Not every time "logical" Clusters see above
		...
		/*
		 * List<double[][]> logicalMatrices = new LinkedList<double[][]>();
		 * zwischen = logicalAdjacentMatrix.clone();
		 * logicalMatrices.add(zwischen.clone());
		 * 
		 * for (int i = 1; i <= internalMaxID + 1; i ++) { zwischen =
		 * Matrix.multiplyByMatrix(zwischen, logicalAdjacentMatrix);
		 * logicalMatrices.add(zwischen.clone()); //System.out.println("i = " +
		 * i); //System.out.println(Matrix.toString(zwischen)); } for (Integer
		 * controlID : controlCenters) { int internalControlID =
		 * externalToInternalID.get(controlID); Double[] connectionAvailable =
		 * new Double[internalMaxID + 1]; for (int i = 0; i <
		 * connectionAvailable.length; i++) { connectionAvailable[i] = 0.0; }
		 * for (double[][] m : logicalMatrices) { for (int node = 0; node <
		 * internalMaxID + 1; node++) { if (m[internalControlID][node] > 0)
		 * connectionAvailable[node] = 1.0; } }
		 * 
		 * System.out.println("ControlCenter " + controlID + " can connect to "
		 * ); for (int i = 0; i < connectionAvailable.length; i++) { if
		 * (connectionAvailable[i] > 0 && i != internalControlID)
		 * System.out.println(internalToExternalID.get(i)); }
		 * controlCenterConnectivity.put(controlID, connectionAvailable); }
		 */
	...
	private void clusterCleaning(SmartgridoutputFactory factory,
		...
		// Cluster die danach keine Elemente mehr haben werden gel̦scht
	...
	private boolean externalNodeHasPower(int id) {
		...
		for (PowerGridNode pgn : entityStates.get(id).getOwner()
				.getConnectedTo()) {
	...
	private boolean areInSameCluster(int n, int m,
			List<List<Integer>> clusterList) {
