                    ...
                    protected void doExecute() {
                        ...
                        for (final EObject current: boList) {
                            if (current instanceof Scenario) {
                                if (!(LoadInputModelConformityHelper.checkInputModelConformity((ScenarioState) obj, (Scenario) current))) {
                                    MessageBox messageBox = new MessageBox(shell, SWT.ICON_ERROR);
                        ...
                        Shape[] shapes = (Shape[]) diagramContainer.getDiagramTypeProvider().getDiagram().getChildren()
                            ...
                            for (int i = 0; i < shapes.length; i++) {
                                ...
                                if (o instanceof PowerGridNode) {
                                    Object ob = resolveBOfromPowergridNode((PowerGridNode) containerShape.getLink()
                                            .getBusinessObjects().get(0), ((ScenarioState) obj).getPowerStates());
                                    ...
                                    if (ob instanceof PowerState && ((PowerState) ob).isPowerOutage()) {
                                        containerShape.getGraphicsAlgorithm().setBackground(
                                                manager.manageBackground(true));

                                    } else if (ob instanceof PowerState) {
                                        containerShape.getGraphicsAlgorithm().setBackground(
                                                manager.manageBackground(false));
                                    }
                                } else if (o instanceof NetworkEntity) {
                                    Object ob = resolveBOfromNetworkEntity((NetworkEntity) containerShape.getLink()
                                            .getBusinessObjects().get(0), ((ScenarioState) obj).getEntityStates());
                                    ...
                                    } else if (ob != null && ob instanceof EntityState && containerShape instanceof ContainerShape) {
