...
public class InputModelCreator {
	 ...
	 * @param isDefault if parameter is true: a default model will becreated
	...
	public int createNewInputModel(boolean isDefault) {
		...
		if(!isDefault) {
			...
			// TODO clear button muss enabled werden -> schwierig -> deshalb custom actions disabled solange kein input modell geladen ist 
		...
		RecordingCommand c = new RecordingCommand(domain) {
			...
			protected void doExecute() {
		        ...
		        EObject domainModel = SmartgridinputFactory.eINSTANCE.createScenarioState();
		        ...
		        for (Shape shape : shapes) {
		        	...
		        	if (obj instanceof NetworkEntity) {
		        		...
		        		((ScenarioState)domainModel).getEntityStates().add(state);
		        	}
		        	if (obj instanceof PowerGridNode) {
		        		...
		        		((ScenarioState)domainModel).getPowerStates().add(state);
		        	}
		        }
	 ...
	 * @param shell the current swt shell
