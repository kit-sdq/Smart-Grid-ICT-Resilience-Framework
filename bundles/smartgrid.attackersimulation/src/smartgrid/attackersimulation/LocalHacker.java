...
public class LocalHacker implements IAttackerSimulation {
	...
	public void init(final ILaunchConfiguration config) throws CoreException {
		...
		switch (hackingTypes) {
		case BFS_HACKING:
			this.hackingTypes = new BFSStrategy(ignoreLogicalConnections, hackingSpeed);
			break;
		case DFS_HACKING:
			this.hackingTypes = new DFSStrategy(ignoreLogicalConnections, hackingSpeed);
			break;
		case FULLY_MESHED_HACKING:
			this.hackingTypes = new FullyMeshedStrategy(hackingSpeed);
	...
	public ScenarioResult run(final SmartGridTopology smartGridTopo, final ScenarioResult scenarioResult) {
		...
		if (this.rootNodeState != null) {
			hackingTypes.hackNextNode(rootNodeState);
		}
