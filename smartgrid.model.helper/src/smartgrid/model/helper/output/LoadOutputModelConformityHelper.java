...
public final class LoadOutputModelConformityHelper {
    ...
    public static boolean checkOutputModelConformitySimple(ScenarioResult result, Scenario current) {
    ...
    public static boolean checkOutputModelConformity(ScenarioResult output, Scenario current) {
        // Check differences that would be obvious and ignore if output.getScenario() is null
        boolean result = checkOutputModelConformitySimple(output, current);
         
        if (result) {
            return compareAndCountEntityStates(output.getEntityStates(), current.getContainsNE());
        }
